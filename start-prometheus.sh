...
sleep 10
helm install -f config/helm/prometheus/values.yaml prometheus prometheus-community/prometheus
sleep 60
...
POD_NAME=$(kubectl get pods --namespace default -l "app.kubernetes.io/name=alertmanager" -o jsonpath="{.items[0].metadata.name}")
